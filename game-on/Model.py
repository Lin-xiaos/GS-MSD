...
# 模态间不一致学习模块
def calculate_js_divergence(text_distribution, image_distribution):
    """
    计算文本和图像模态之间的 JS 散度。
    :param text_distribution: 文本模态的分布（张量）
    :param image_distribution: 图像模态的分布（张量）
    :return: 两者之间的 JS 散度
    """
    # 转换为张量并确保类型
    ...
    # 计算 JS 散度（按常见定义归一化）
    js_divergence = (kl_text_m + kl_image_m) / (2 * np.log(2))

    return js_divergence


class CrossModalAttention(nn.Module):
    def __init__(self, feature_dim=256, num_modalities=1):
        super().__init__()
        # 保存输入参数
    ...
    def forward(self, shared_features, text_features):
        # 检查输入特征的维度
        for features in [shared_features, text_features]:
            assert features.size(-1) == self.feature_dim, f"Input feature dimension must be {self.feature_dim}"

        features = [shared_features, text_features]
        # 对所有特征进行变换
...
class MModel(nn.Module):
    ...
    def forward(self, g, text_graph, image_graph):
        x = g.ndata['features']
        ...
        x_cross = self.cross_modal_attention(x_fused, image_feature)
        ...
        js_divergence_loss = calculate_js_divergence(text_feature, image_feature)
        # return sar, sent, emo
